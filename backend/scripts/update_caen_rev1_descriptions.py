"""
Update missing CAEN Rev.1 code descriptions using ANAF correspondence table.
Maps Rev.1 codes to their proper Romanian descriptions.
"""
import pymongo
import os
import sys

# Rev.1 code -> description (extracted from ANAF Rev.2 correspondence table)
# Format: Rev.1_code: "Description (from Rev.2 equivalent)"
REV1_DESCRIPTIONS = {
    # Comert cu amanuntul - sectiunea G (cele mai frecvente)
    ...
    # Industria prelucratoare diverse
...
def run_update():
    ...
    for code, description in REV1_DESCRIPTIONS.items():
        result = db.caen_codes.update_one(
            {"cod": code, "$or": [
                {"name": {"$regex": "^Cod CAEN "}},
                {"name": {"$exists": False}},
                {"name": ""},
    ...
    # Also upsert codes that may not exist in DB at all
    upserted = 0
    for code, description in REV1_DESCRIPTIONS.items():
        existing = db.caen_codes.find_one({"cod": code})
        if not existing:
            # Determine section
            try:
                d = int(code[:2])
                ...
                continue
            section_letter = ""
            if 1 <= d <= 3: section_letter = "A"
            elif 5 <= d <= 9: section_letter = "B"
            elif 10 <= d <= 33: section_letter = "C"
            elif d == 35: section_letter = "D"
            elif 36 <= d <= 39: section_letter = "E"
            elif 41 <= d <= 43: section_letter = "F"
            elif 45 <= d <= 47: section_letter = "G"
            elif 49 <= d <= 53: section_letter = "H"
            elif 55 <= d <= 56: section_letter = "I"
            elif 58 <= d <= 63: section_letter = "J"
            elif 64 <= d <= 66: section_letter = "K"
            elif d == 68: section_letter = "L"
            elif 69 <= d <= 75: section_letter = "M"
            elif 77 <= d <= 82: section_letter = "N"
            elif d == 84: section_letter = "O"
            elif d == 85: section_letter = "P"
            elif 86 <= d <= 88: section_letter = "Q"
            elif 90 <= d <= 93: section_letter = "R"
            elif 94 <= d <= 96: section_letter = "S"
            elif 97 <= d <= 98: section_letter = "T"
            elif d == 99: section_letter = "U"

            db.caen_codes.insert_one({
                "cod": code,
                "name": description,
                "description": description,
                "denumire": description,
    ...
    generic_count = db.caen_codes.count_documents({"name": {"$regex": "^Cod CAEN \\d"}})
    print(f"Updated with proper descriptions: {updated}")
    print(f"Upserted new codes: {upserted}")
    print(f"Total caen_codes: {final_count}")
    print(f"Remaining with generic description: {generic_count}")

    # Verify
    print("\n=== VERIFICATION ===")
    for code in ["5211", "4100", "5262", "4521", "4933", "6210", "7020", "9999", "5248", "5142", "5611", "5541"]:
        d = db.caen_codes.find_one({"cod": code}, {"_id": 0, "cod": 1, "name": 1})
        if d:
            name = d.get("name", "N/A")
            is_generic = name.startswith("Cod CAEN ")
            status = "GENERIC" if is_generic else "OK"
            print(f"  {status}  {d['cod']}: {name[:70]}")
        else:
